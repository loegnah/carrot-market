...
import type { User, Answer, Post } from "@prisma/client";
...
const CommunityPostDetail: NextPage = () => {
  ...
  const onWonderClick = () => {
    ...
    mutate(
      _.chain(data)
        .set(
          "post._count.wondering",
          data.isWondering
            ? data?.post._count.wondering - 1
            : data?.post._count.wondering + 1
        )
        .set("isWondering", !data.isWondering)
        .value(),
      false
    );
    if (!loading) {
            ...
            <div key={answer.id} className="flex items-start space-x-3">
              ...
              <div>
                ...
                <span className="text-xs text-gray-500 block ">
                  {answer.createdAt.toString().replace("T", " ").slice(0, -1)}
                </span>
